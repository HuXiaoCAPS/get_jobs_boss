...
> [!IMPORTANT]
> **这是 [`loks666/get_jobs`](https://github.com/loks666/get_jobs) 的个人改造 fork，不是上游原件。**
>
> 与上游的区别：**只保留 Boss 直聘**、配置改为 **`config/boss.yaml` 文件驱动**、
> 投递流程重构为**平台可插拔**（缺平台也能启动运行）、前端重组为「投递 / 配置 / 数据 / 外观」四个页面。
>
> 👉 **改了什么、后续计划、怎么跟上上游：见 [FORK.md](./FORK.md)**
>
> 需要猎聘 / 前程无忧 / 智联招聘，或想要原版多平台与界面，请直接用[上游仓库](https://github.com/loks666/get_jobs)。
...
> 本 fork 的界面是「只做 Boss」的四页版本（投递 / 配置 / 数据 / 外观），
> 上游那批截图（大多是已从本 fork 删除的猎聘 / 51job / 智联界面）不再适用，已移除。
> 界面与配置说明见 [FORK.md](./FORK.md)。
---
