...
public interface JobPlatform {
    ...
    /**
     * 用<b>本平台的浏览器</b>打开一个地址（网页端「数据」页的「在浏览器中打开」）。
     *
     * <p>为什么需要它：管理页跑在用户自己的浏览器里，而那里<b>没有</b>自动化 profile 的登录态 ——
     * 点开岗位详情只会看到登录页。只有平台自己的浏览器（Boss 用持久化 profile
     * {@code browser-data/}）才带着登录态，所以这个动作属于平台能力。
     *
     * <p>默认返回 false（纯 HTTP 平台的实现里没有"浏览器"这个概念）。
     * <b>校验地址是平台的责任</b>：实现方要自己确认这个 URL 属于本平台，
     * 否则网页端就成了"用已登录的浏览器打开任意地址"的跳板。
     *
     * @return 是否成功打开
     */
    default boolean openInBrowser(String url) {
        return false;
    }
}
